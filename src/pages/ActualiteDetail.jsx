  ...
  if (actualite === null) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6">
        <h1 className="text-2xl font-bold text-navy">Actualité introuvable</h1>
        <p className="mt-2 text-slate-600">Cette actualité n'existe pas ou a été retirée.</p>
        <Button as={Link} to="/actualites" className="mt-6">Retour aux actualités</Button>
      </div>
    );
  }
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      <div className="mt-6 flex items-center gap-3">
        <Badge tone="cobalt">{actualite.categorie}</Badge>
        <span className="text-sm text-slate-400">{formatDate(actualite.date)}</span>
      </div>
      <h1 className="mt-4 text-3xl font-extrabold leading-tight text-navy">
        {actualite.titre}
      </h1>
      <img
        src={actualite.image}
        alt=""
        className="mt-8 h-72 w-full rounded-card object-cover"
      />
