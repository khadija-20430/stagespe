...
router.post('/upload', verifyToken, checkRole('super_admin', 'admin'), (req, res) => {
...
router.get('/expired', verifyToken, checkRole('super_admin', 'admin'), async(req, res) => {
    try {
...
router.get('/:id', async(req, res) => {
    try {
        const result = await pool.query(
            `SELECT documents.*, document_categories.code AS categorie_code, document_categories.label AS categorie_label
       FROM documents JOIN document_categories ON documents.categorie_id = document_categories.id
       WHERE documents.id = $1`, [req.params.id]
        );
        ...
        await pool.query(
            'INSERT INTO document_access_logs (document_id, user_id, ip_address, user_agent, action) VALUES ($1,$2,$3,$4,$5)', [req.params.id, null, req.ip, req.headers['user-agent'], 'view']
        );
...
router.get('/:id/download', async(req, res) => {
    try {
        ...
        await pool.query(
            'INSERT INTO document_access_logs (document_id, user_id, ip_address, user_agent, action) VALUES ($1,$2,$3,$4,$5)', [req.params.id, null, req.ip, req.headers['user-agent'], 'download']
        );
...
router.get('/:id/revisions', verifyToken, checkRole('super_admin', 'admin'), async(req, res) => {
    try {
        const result = await pool.query(
       ...
       WHERE document_id = $1 ORDER BY created_at DESC`, [req.params.id]
        );
        res.json(result.rows);
    } catch (err) { sendError(res, err); }
});
...
router.post('/:id/link', verifyToken, checkRole('super_admin', 'admin'), async(req, res) => {
    try {
        ...
        await pool.query(
            `INSERT INTO ${config.table} (document_id, ${config.fk}) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [req.params.id, entity_id]
        );
...
router.delete('/:id/link', verifyToken, checkRole('super_admin', 'admin'), async(req, res) => {
    try {
        const { entity_type, entity_id } = req.body;
        const config = LINK_TABLES[entity_type];
...
router.put('/:id', verifyToken, checkRole('super_admin', 'admin'), async(req, res) => {
    try {
        ...
        if (fichier_url && fichier_url !== existing.rows[0].fichier_url) {
            await pool.query(
                `INSERT INTO document_revisions (document_id, version, fichier_url, file_size, changed_by, change_note) VALUES ($1,$2,$3,$4,$5,$6)`, [req.params.id, existing.rows[0].version, existing.rows[0].fichier_url, existing.rows[0].file_size, req.user.id, change_note || null]
            );
        }
       ...
       WHERE id=$12 RETURNING *`, [titre, description, fichier_url, categorie_id, langage, version, file_size, file_format, visibilite, is_featured, date_expiration, req.params.id]
        );
