...
async function translateList(entityType, rows, langCode) {
  ...
  const byEntityId = {};
  for (const row of result.rows) byEntityId[row[config.fk]] = row;

  return rows.map((row) => {
    const t = byEntityId[row.id];
    if (!t) return row;
    const merged = { ...row };
    for (const field of config.fields) {
      if (t[field] !== undefined && t[field] !== null && t[field] !== '') merged[field] = t[field];
    }
    return merged;
  });
}
...
async function upsertTranslations(entityType, entityId, translationsInput) {
  const config = TRANSLATION_CONFIG[entityType];
  if (!config || !translationsInput || typeof translationsInput !== 'object') return;
  ...
  for (const langCode of Object.keys(translationsInput)) {
    ...
    const relevantFields = config.fields.filter((f) => fieldsInput[f] !== undefined);
    if (relevantFields.length === 0) continue;

    const mainField = config.fields[0];
    const columns = [config.fk, 'language_id', ...config.fields];
    const values = [entityId, languageId, ...config.fields.map((f) => (f === mainField ? (fieldsInput[f] || '') : fieldsInput[f] ?? null))];
...
module.exports = { translateList, translateOne, upsertTranslations, getAllTranslations, deleteTranslations };
